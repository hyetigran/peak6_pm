...
( cd services/indexer && INDEXER_DB=.indexer.sqlite PORT=8787 DEMO_FAUCET="$ROOT/.demo-faucet.json" DEMO_CONFIG="$ROOT/.demo-config.json" npx tsx src/index.ts >/tmp/idx.log 2>&1 & echo $! >/tmp/idxpid )
sleep 5
echo "=== /book/$MKT ==="
