...
keys. **Always put the public key in a key file's name** (`keys/<role>-<pubkey>.json`); the seed
writes one per authority it uses and never overwrites an existing one. Context: on 2026-08-23 a localnet `make demo` overwrote the devnet governance/operator
